...
int decompress_onepass (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
	...
	for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row; yoffset++) 
	{
		for (MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col; MCU_col_num++) 
		{
			...
			jzero_far((void *) coef->MCU_buffer[0],	(size_t) (cinfo->blocks_in_MCU * sizeof(JBLOCK)));
			...
			for (ci = 0; ci < cinfo->comps_in_scan; ci++) 
			{
				...
				for (yindex = 0; yindex < compptr->MCU_height; yindex++) 
				{
						...
						yoffset+yindex < compptr->last_row_height) 
					{
						...
						{
							(*inverse_DCT) (cinfo, compptr, (JCOEFPTR) coef->MCU_buffer[blkn+xindex],
...
int decompress_data (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
	...
	for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) 
	{
		...
		buffer = cinfo->mem->access_virt_barray(coef->whole_image[ci],
...
smoothing_ok (j_decompress_ptr cinfo)
{
	...
	if (coef->coef_bits_latch == NULL)
		coef->coef_bits_latch = (int *) cinfo->mem->alloc_small(JPOOL_IMAGE,
...
jinit_d_coef_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
	...
	coef = (my_coef_ptr) cinfo->mem->alloc_small (JPOOL_IMAGE, sizeof(my_coef_controller));
	...
	else 
	{
		...
		JBLOCKROW buffer = (JBLOCKROW) cinfo->mem->alloc_large(JPOOL_IMAGE, D_MAX_BLOCKS_IN_MCU * sizeof(JBLOCK));
