...
void start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	cinfo->ERREXIT1(JERR_NO_HUFF_TABLE, dctbl);
      if (actbl < 0 || actbl >= NUM_HUFF_TBLS)
	cinfo->ERREXIT1(JERR_NO_HUFF_TABLE, actbl);
      /* Allocate and zero the statistics tables */
      /* Note that jpeg_gen_optimal_table expects 257 entries in each table! */
      if (entropy->dc_count_ptrs[dctbl] == NULL)
	entropy->dc_count_ptrs[dctbl] = (long *)
	  cinfo->mem->alloc_small (JPOOL_IMAGE, 257 * sizeof(long));
			 ...
			 c_derived_tbl ** pdtbl)
{
  ...
  /* Allocate a workspace if we haven't already done so. */
	if (*pdtbl == NULL)
		*pdtbl = (c_derived_tbl *)
						cinfo->mem->alloc_small (JPOOL_IMAGE, sizeof(c_derived_tbl));
	dtbl = *pdtbl;
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  if (size == 0)
    state->cinfo->ERREXIT(JERR_HUFF_MISSING_CODE);
		  ...
		  c_derived_tbl *dctbl, c_derived_tbl *actbl)
{
  ...
  if (nbits > MAX_COEF_BITS+1)
    state->cinfo->ERREXIT(JERR_BAD_DCT_COEF);
		 ...
		 long dc_counts[], long ac_counts[])
{
   ...
   */
  if (nbits > MAX_COEF_BITS+1)
    cinfo->ERREXIT(JERR_BAD_DCT_COEF);
...
#define MAX_CLEN 32		/* assumed maximum initial code length */
  ...
  for (i = 0; i <= 256; i++) {
	...
	cinfo->ERREXIT(JERR_HUFF_CLEN_OVERFLOW);
...
jinit_huff_encoder (j_compress_ptr cinfo)
{
	...
	entropy = (huff_entropy_ptr)
		cinfo->mem->alloc_small (JPOOL_IMAGE, sizeof(huff_entropy_encoder));
