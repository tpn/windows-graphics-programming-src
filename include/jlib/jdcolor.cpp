...
build_ycc_rgb_table (j_decompress_ptr cinfo)
{
  ...
  cconvert->Cr_r_tab = (int *)   cinfo->mem->alloc_small(JPOOL_IMAGE, (MAXJSAMPLE+1) * sizeof(int));
  cconvert->Cb_b_tab = (int *)   cinfo->mem->alloc_small(JPOOL_IMAGE, (MAXJSAMPLE+1) * sizeof(int));
  cconvert->Cr_g_tab = (long *) cinfo->mem->alloc_small(JPOOL_IMAGE, (MAXJSAMPLE+1) * sizeof(long));
  cconvert->Cb_g_tab = (long *) cinfo->mem->alloc_small(JPOOL_IMAGE, (MAXJSAMPLE+1) * sizeof(long));
...
void gray_rgb_convert (j_decompress_ptr cinfo,
		  JSAMPIMAGE input_buf, JDIMENSION input_row,
		  JSAMPARRAY output_buf, int num_rows)
{
  register JSAMPROW inptr, outptr;
...
jinit_color_deconverter (j_decompress_ptr cinfo)
{
  ...
  cconvert = (my_cconvert_ptr)
    cinfo->mem->alloc_small(JPOOL_IMAGE, sizeof(my_color_deconverter));
  ...
  case JCS_GRAYSCALE:
    if (cinfo->num_components != 1)
      cinfo->ERREXIT(JERR_BAD_J_COLORSPACE);
    break;
  ...
  case JCS_GRAYSCALE:
	...
	cinfo->comp_info[ci].component_needed = FALSE;
      ...
      cinfo->ERREXIT(JERR_CONVERSION_NOTIMPL);
    break;
