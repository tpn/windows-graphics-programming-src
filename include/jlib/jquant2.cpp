...
void start_pass_2_quant (j_decompress_ptr cinfo, boolean is_pre_scan)
{
  ...
  if (is_pre_scan) {
    ...
    if (i < 1)
      cinfo->ERREXIT1(JERR_QUANT_FEW_COLORS, 1);
    if (i > MAXNUMCOLORS)
      cinfo->ERREXIT1(JERR_QUANT_MANY_COLORS, MAXNUMCOLORS);
...
jinit_2pass_quantizer (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->enable_2pass_quant) {
    ...
    if (desired < 8)
      cinfo->ERREXIT1(JERR_QUANT_FEW_COLORS, 8);
    /* Make sure colormap indexes can be represented by JSAMPLEs */
    if (desired > MAXNUMCOLORS)
      cinfo->ERREXIT1(JERR_QUANT_MANY_COLORS, MAXNUMCOLORS);
    cquantize->sv_colormap = cinfo->mem->alloc_sarray
      (JPOOL_IMAGE, (JDIMENSION) desired, (JDIMENSION) 3);
