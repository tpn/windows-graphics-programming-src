  ...
  wsptr = workspace;
	...
	for (int ctr = DCTSIZE; ctr > 0; ctr--) 
	{
		...
		*/
		...
		long z2 = DEQUANTIZE(inptr[DCTSIZE*2], quantptr[DCTSIZE*2]);
		long z3 = DEQUANTIZE(inptr[DCTSIZE*6], quantptr[DCTSIZE*6]);
    	long z1 = (z2 + z3) * FIX_0_541196100;
    
		long tmp2 = z1 + MULTIPLY(z3, - FIX_1_847759065);
		long tmp3 = z1 + MULTIPLY(z2, FIX_0_765366865);
		...
		*/
		...
		z3 = tmp0 + tmp2;
		long z4 = tmp1 + tmp3;
		long z5 = MULTIPLY(z3 + z4, FIX_1_175875602); /* sqrt(2) * c3 */
	...
	for (ctr = 0; ctr < DCTSIZE; ctr++) 
	{
		...
		*/
		...
		long z2 = (long) wsptr[2];
		long z3 = (long) wsptr[6];
		long z1 = MULTIPLY(z2 + z3, FIX_0_541196100);
    
		long tmp2 = z1 + MULTIPLY(z3, - FIX_1_847759065);
		long tmp3 = z1 + MULTIPLY(z2, FIX_0_765366865);
    
		long tmp0 = ((long) wsptr[0] + (long) wsptr[4]) << CONST_BITS;
		long tmp1 = ((long) wsptr[0] - (long) wsptr[4]) << CONST_BITS;
